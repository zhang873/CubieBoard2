...
int Volume::mountVol() {
    ...
    for (i = 0; i < n; i++) {
        ...
        {
             if( Fat::doMount(devicePath, "/mnt/secure/staging", false, false, false, AID_SYSTEM, gid, 0002, true) )
             {
        ...
        else if(!iso9660::doMount(devicePath, "/mnt/secure/staging", false, false, false, AID_SYSTEM, gid, 0702, false) )
        {
        ...
        SLOGE("Device %s, target %s mounted @ /mnt/secure/staging", devicePath, getMountpoint());
...
int Volume::doUnmount(const char *path, bool force) {
    ...
    while (retries--) {
        ...
        if (force) {
            //add by chenjd,sometime it may spend more than 2s to kill the process,so we make it enought time to do this.
            if (retries == 4) {
                action = 2; // SIGKILL
            } else if (retries == 8) {
                action = 1; // SIGHUP
