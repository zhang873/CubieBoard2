...
#define TEST_FORCE_GPU_RENDER   1//zhangxc mod 0->1
...
status_t CedarXPlayer::setNativeWindow_l(const sp<ANativeWindow> &native) {
	...
	if(mNativeWindow == NULL && (mFlags&PREPARED))	//* setSurface() is called after prepareAsync(), play not start yet.
	{
	    ...
	    //0: no rotate, 1: 90 degree (clock wise), 2: 180, 3: 270, 4: horizon flip, 5: vertical flig;
	      LOGD("parker------------------------mInitRotation = %d", mInitRotation);
	      if(mInitRotation)
	      {
	        if(mRenderToDE)
	        {
	      LOGD("parker------------------------mRenderToDE = %d", mRenderToDE);
	        	//* player has been prepared, set rotate may be not work, check it later.
	            LOGD("init rotate[%d] in setNativeWindow_l()!", mInitRotation);
	            mPlayer->control(mPlayer, CDX_CMD_SET_VIDEO_ROTATION, 3, 0);
	        }
	}
	}
...
status_t CedarXPlayer::prepareAsync() {
    ...
    //0: no rotate, 1: 90 degree (clock wise), 2: 180, 3: 270, 4: horizon flip, 5: vertical flig;
	      LOGD("2222222222222parker------------------------mInitRotation = %d", mInitRotation);
	if(mInitRotation)
	{
        if(mRenderToDE)
        {
	      LOGD("22222222222222222222parker------------------------mRenderToDE = %d", mRenderToDE);
...
status_t CedarXPlayer::generalInterface(int cmd, int int1, int int2, int int3, void *p)
{
	...
	if (mPlayer == NULL) {
     ...
     LOGD("parker-----7777777777777777777777--------------cmd = [%d] int1 = %d int2 = %d  int3 = %d *p = %d c*p = %s p*p = %p", cmd, int1, int2, int3, p, p, p);
     cmd= 0;
     if(int1 == 3) {
        mInitRotation = 3;
        int1 = 0;	
...
int CedarXPlayer::CedarXPlayerCallback(int event, void *info)
{
	...
	case CDX_MEDIA_BUFFERING_PRECENT:
	{
		LOGV("cache percentage %d MEDIA_INFO_CACHE_PRECENT=%d", (int)para,MEDIA_INFO_CACHE_PRECENT);
		notifyListener_l(MEDIA_INFO,MEDIA_INFO_CACHE_PRECENT, (int)para);
		break;
	}
	...
	case CDX_EVENT_PREPARED:
         {
	          ...
	          LOGV("pCDXPrepareDoneInfo->_3d_mode_double_stream_flag =%d",pCDXPrepareDoneInfo->_3d_mode_double_stream_flag);
	          if(pCDXPrepareDoneInfo->_3d_mode_double_stream_flag)
              {
