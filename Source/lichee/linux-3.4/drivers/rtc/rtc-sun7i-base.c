...
static int i2c_write_reg2(struct i2c_adapter *adapter, u8 adr,
			 u8 reg, u8 data)
{
	u8 m[2] = {reg, data};
...
static int i2c_read_reg2(struct i2c_adapter *adapter, u8 adr,
			u8 reg, u8 *val)
{
...
retry_get_time:
	...
	//hhmmss = rtc_reg_list->r_hh_mm_ss; //zhangxc
	
	adap = i2c_get_adapter(1);
	if (adap != NULL) {
		i2c_read_reg2(adap, addr, 0x00, &val);
		pr_info("########## 0x00 var: %x\n", val);
		hhmmss.second = ((val & 0xF0) >> 4) * 10 + (val & 0x0F);
		
		i2c_read_reg2(adap, addr, 0x01, &val);
		pr_info("########## 0x01 var: %x\n", val);
		hhmmss.minute = ((val & 0xF0) >> 4) * 10 + (val & 0x0F);
		
		i2c_read_reg2(adap, addr, 0x02, &val);
		pr_info("########## 0x02 var: %x\n", val);
		hhmmss.hour = ((val & 0xF0) >> 4) * 10 + (val & 0x0F);
		
		i2c_read_reg2(adap, addr, 0x04, &val);
		pr_info("########## 0x04 var: %x\n", val);
		yymmdd.day = ((val & 0xF0) >> 4) * 10 + (val & 0x0F);
		
		i2c_read_reg2(adap, addr, 0x05, &val);
		pr_info("########## 0x05 var: %x\n", val);
		yymmdd.month = ((val & 0xF0) >> 4) * 10 + (val & 0x0F);
		
		i2c_read_reg2(adap, addr, 0x06, &val);
		pr_info("########## 0x06 var: %x\n", val);
		yymmdd.year = ((val & 0xF0) >> 4) * 10 + (val & 0x0F);
	}
	pr_info("%s(%d) get: read time %d-%d-%d %d:%d:%d\n", __func__, __LINE__, yymmdd.year,
		yymmdd.month, yymmdd.day, hhmmss.hour, hhmmss.minute, hhmmss.second);
	
	rtc_tm->tm_sec  = hhmmss.second;
	rtc_tm->tm_min  = hhmmss.minute;
	rtc_tm->tm_hour = hhmmss.hour;

	//yymmdd = rtc_reg_list->r_yy_mm_dd; //zhangxc
	rtc_tm->tm_mday = yymmdd.day;
	rtc_tm->tm_mon  = yymmdd.month;
	rtc_tm->tm_year = yymmdd.year + 100;
...
static int sunxi_rtc_settime(struct device *dev, struct rtc_time *tm)
{
	...
	pr_info("%s(%d) set : write time %d-%d-%d %d:%d:%d\n", __func__, __LINE__, yymmdd.year,
		yymmdd.month, yymmdd.day, hhmmss.hour, hhmmss.minute, hhmmss.second);
	adap = i2c_get_adapter(1);
	if (adap != NULL) {
		i2c_write_reg2(adap, addr, 0x00, ((hhmmss.second / 10) << 4) + (hhmmss.second % 10));
		i2c_write_reg2(adap, addr, 0x01, ((hhmmss.minute / 10) << 4) + (hhmmss.minute % 10));
		i2c_write_reg2(adap, addr, 0x02, ((hhmmss.hour / 10) << 4) + (hhmmss.hour % 10));
		
		i2c_write_reg2(adap, addr, 0x04, ((yymmdd.day / 10) << 4) + (yymmdd.day % 10));
		i2c_write_reg2(adap, addr, 0x05, ((yymmdd.month / 10) << 4) + (yymmdd.month % 10));
		i2c_write_reg2(adap, addr, 0x06, (((yymmdd.year - 100) / 10) << 4) + ((yymmdd.year - 100) % 10));
	}
